...
char * chared_ammo_name[PP2_MAX_PAINTBALL_TYPES] = 
{
	"PP2_PAINTBALL_TYPE_NORMAL",
	"PP2_PAINTBALL_TYPE_SPLITTER",
	"PP2_PAINTBALL_TYPE_MINE",
	"PP2_PAINTBALL_TYPE_BOUNCER",
	"PP2_PAINTBALL_TYPE_SEEKER",
	"PP2_PAINTBALL_TYPE_REFLECTOR",
	"PP2_PAINTBALL_TYPE_PMINE",
	"PP2_PAINTBALL_TYPE_GHOST"
};
...
void chared_animations_logic(void)
{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_INSERT) || t3f_key_pressed(ALLEGRO_KEY_EQUALS))
	{
...
//			chared_remove_filename(chared_ani_path);
		...
		if(chared_file_load_ani_dialog)
		{
			...
			if(al_get_native_file_dialog_count(chared_file_load_ani_dialog) > 0)
			{
				for(i = 0; i < al_get_native_file_dialog_count(chared_file_load_ani_dialog); i++)
				{
					...
					chared_character->animation[chared_character->animations] = t3f_load_animation(chared_ani_path, 0, false);
	...
	if(t3f_key_pressed(ALLEGRO_KEY_ENTER))
	{
		...
		if(al_get_native_file_dialog_count(chared_file_load_ani_dialog) > 0)
		{
			...
			ap = t3f_load_animation(al_path_cstr(chared_replace_path(chared_return_path_cstr), '/'), 0, false);
			if(ap)
			{
				t3f_destroy_animation(chared_character->animation[chared_selected_animation]);
	...
	{
		for(i = 0; i < chared_character->animation[chared_selected_animation]->data->frames; i++)
		{
			chared_character->animation[chared_selected_animation]->data->frame[i]->x *= 2;
			chared_character->animation[chared_selected_animation]->data->frame[i]->y *= 2;
			chared_character->animation[chared_selected_animation]->data->frame[i]->width *= 2;
			chared_character->animation[chared_selected_animation]->data->frame[i]->height *= 2;
		}
...
void chared_character_logic(void)
{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_LEFT))
	{
		...
		else if(t3f_key_held(ALLEGRO_KEY_P))
		{
			chared_selected_ammo_type--;
			if(chared_selected_ammo_type < 0)
			{
				chared_selected_ammo_type = PP2_MAX_PAINTBALL_TYPES - 1;
			}
		}
		else
		{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_RIGHT))
	{
		...
		{
			chared_selected_ammo_type++;
			if(chared_selected_ammo_type >= PP2_MAX_PAINTBALL_TYPES)
			{
				chared_selected_ammo_type = 0;
			}
		}
		else
		{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_COMMA))
	{
		chared_character->state[chared_selected_state].piece[chared_selected_piece].angle -= 0.01;
		t3f_use_key_press(ALLEGRO_KEY_COMMA);
	}
	if(t3f_key_pressed(ALLEGRO_KEY_FULLSTOP))
	{
		chared_character->state[chared_selected_state].piece[chared_selected_piece].angle += 0.01;
		t3f_use_key_press(ALLEGRO_KEY_FULLSTOP);
	}
	...
	if(t3f_key_pressed(ALLEGRO_KEY_M))
	{
	...
	if(t3f_key_held(ALLEGRO_KEY_LSHIFT) && t3f_key_pressed(ALLEGRO_KEY_P))
	{
		facing = (chared_selected_state / 8) % 2;
		for(i = chared_selected_state % 8; i < PP2_CHARACTER_MAX_STATES; i += 8)
		{
	...
	if(t3f_mouse_button_held(0))
	{
		...
		if(t3f_key_pressed(ALLEGRO_KEY_P))
		{
			if(t3f_key_held(ALLEGRO_KEY_OPENBRACE))
			{
		...
		else if(t3f_key_held(ALLEGRO_KEY_LSHIFT))
		{
			if(t3f_key_held(ALLEGRO_KEY_LCTRL))
			{
				chared_character->state[chared_selected_state].collision_w += (t3f_get_mouse_x() - pegx);
				chared_character->state[chared_selected_state].collision_h += (t3f_get_mouse_y() - pegy);
			}
			else
			{
				chared_character->state[chared_selected_state].collision_x += (t3f_get_mouse_x() - pegx);
				chared_character->state[chared_selected_state].collision_y += (t3f_get_mouse_y() - pegy);
			}
		}
		else
		{
			chared_character->state[chared_selected_state].piece[chared_selected_piece].x += (t3f_get_mouse_x() - pegx);
			chared_character->state[chared_selected_state].piece[chared_selected_piece].y += (t3f_get_mouse_y() - pegy);
		}
		pegx = t3f_get_mouse_x();
		pegy = t3f_get_mouse_y();
	}
...
void chared_move_bitmap(int bitmap, float ox, float oy)
{
	...
	for(i = 0; i < chared_character->animation[chared_selected_animation]->data->frames; i++)
	{
		if(chared_character->animation[chared_selected_animation]->data->frame[i]->bitmap == bitmap)
		{
			chared_character->animation[chared_selected_animation]->data->frame[i]->x += ox;
			chared_character->animation[chared_selected_animation]->data->frame[i]->y += oy;
		}
	}
}
...
void chared_grab(void)
{
	...
	if(sub_bitmap)
	{
		...
		t3f_destroy_bitmap(chared_character->animation[chared_selected_animation]->data->bitmaps->bitmap[chared_selected_bitmap]);
		chared_character->animation[chared_selected_animation]->data->bitmaps->bitmap[chared_selected_bitmap]->bitmap = al_clone_bitmap(sub_bitmap);
		al_destroy_bitmap(sub_bitmap);
		for(i = 0; i < chared_character->animation[chared_selected_animation]->data->frames; i++)
		{
			if(chared_character->animation[chared_selected_animation]->data->frame[i]->bitmap == chared_selected_bitmap)
			{
				chared_character->animation[chared_selected_animation]->data->frame[i]->width = al_get_bitmap_width(chared_character->animation[chared_selected_animation]->data->bitmaps->bitmap[chared_selected_bitmap]->bitmap);
				chared_character->animation[chared_selected_animation]->data->frame[i]->height = al_get_bitmap_height(chared_character->animation[chared_selected_animation]->data->bitmaps->bitmap[chared_selected_bitmap]->bitmap);
			}
		}
...
void chared_bitmap_logic(void)
{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_G))
	{
		chared_grab();
		t3f_use_key_press(ALLEGRO_KEY_G);
	}
	...
	if(t3f_key_pressed(ALLEGRO_KEY_I))
	{
		...
		{
			for(chared_selected_bitmap = 0; chared_selected_bitmap < chared_character->animation[chared_selected_animation]->data->bitmaps->count; chared_selected_bitmap++)
			{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_UP))
	{
		...
		{
			chared_selected_bitmap = chared_character->animation[chared_selected_animation]->data->bitmaps->count - 1;
		}
		chared_read_import_config();
		t3f_use_key_press(ALLEGRO_KEY_UP);
	}
	...
	{
		chared_selected_bitmap++;
		if(chared_selected_bitmap >= chared_character->animation[chared_selected_animation]->data->bitmaps->count)
		{
			chared_selected_bitmap = 0;
		}
		chared_read_import_config();
	...
	else if(t3f_key_held(ALLEGRO_KEY_LCTRL))
	{
		if(t3f_key_pressed(ALLEGRO_KEY_W))
		{
			chared_import_oy--;
	...
	else
	{
		...
		if(t3f_key_held(ALLEGRO_KEY_A))
		{
			chared_import_vx--;
		}
		if(t3f_key_held(ALLEGRO_KEY_D))
		{
			chared_import_vx++;
		}
	}
	if(t3f_mouse_button_held(0))
	{
		...
		{
			chared_import_ox = t3f_get_mouse_x() + chared_import_vx;
			chared_import_oy = t3f_get_mouse_y() + chared_import_vy;
			chared_import_step = 1;
		}
		else if(chared_import_step == 1)
		{
			chared_import_ow = t3f_get_mouse_x() - chared_import_ox + chared_import_vx;
			chared_import_oh = t3f_get_mouse_y() - chared_import_oy + chared_import_vy;
		}
	}
	else
	{
...
void chared_logic(void * data)
{
	...
	if(t3f_key_pressed(ALLEGRO_KEY_F10))
	{
		int i;
	...
	if(t3f_key_pressed(ALLEGRO_KEY_F11))
	{
		int i;
		...
		t3f_use_key_press(ALLEGRO_KEY_F11);
	}
	if(t3f_key_pressed(ALLEGRO_KEY_F12))
	{
		int i, j, k;
		...
		t3f_use_key_press(ALLEGRO_KEY_F12);
	}
...
{
	t3f_draw_textf(chared_font, t3f_color_white, 0.0, 0.0, 0.0, 0, "View: Animations");
	if(chared_selected_animation < chared_character->animations)
	{
		...
		t3f_draw_textf(chared_font, al_map_rgba(255, 255, 255, 255), 0.0, 216.0, 0.0, 0, "Animation: %d of %d", chared_selected_animation + 1, chared_character->animations);
	}
}
...
void chared_character_render(void)
{
	...
	t3f_draw_textf(chared_font, t3f_color_white, 0.0, 0.0, 0.0, 0, "View: Character");
	...
	t3f_draw_textf(chared_font, al_map_rgba(255, 255, 255, 255), 0.0, 232.0, 0.0, 0, "%s", chared_ammo_name[chared_selected_ammo_type]);
}
...
void chared_bitmap_render(void)
{
	...
	for(i = 0; i < chared_character->animation[chared_selected_animation]->data->frames; i++)
	{
		if(chared_character->animation[chared_selected_animation]->data->frame[i]->bitmap == chared_selected_bitmap)
		{
			...
			t3f_draw_bitmap(chared_character->animation[chared_selected_animation]->data->bitmaps->bitmap[chared_selected_bitmap], t3f_color_white, 640 - 64 + chared_character->animation[chared_selected_animation]->data->frame[i]->x, 480 - 64 + chared_character->animation[chared_selected_animation]->data->frame[i]->y, 0, 0);
			break;
		}
	}
	if(chared_import_animation[chared_selected_animation])
...
void chared_meta_render(void)
{
	switch(chared_selected_meta)
	{
		case 0:
		{
			t3f_draw_textf(chared_font, t3f_color_white, 0, 0, 0, 0, "Name: %s", chared_character->info.name);
			break;
		}
		case 1:
		{
			t3f_draw_textf(chared_font, t3f_color_white, 0, 0, 0, 0, "Author: %s", chared_character->info.author);
			break;
		}
		case 2:
		{
			t3f_draw_textf(chared_font, t3f_color_white, 0, 0, 0, 0, "Comment: %s", chared_character->info.comment);
			break;
		}
	}
}

void chared_render(void * data)
{
	if(chared_character)
	{
	...
	else
	{
		...
		t3f_draw_textf(chared_font, t3f_color_white, 0.0, 0.0, 0.0, 0, "No character loaded...");
	}
}
...
bool chared_initialize(int argc, char * argv[])
{
	...
	chared_font = t3f_load_font("data/fonts/chared_font.png", T3F_FONT_TYPE_AUTO, 0, 0, false);
