...
//	dsy = (float)t3f_display_height / t3f_virtual_display_height;
	...
	if(t3f_current_view->flags & T3F_FORCE_ASPECT)
	{
		...
		if(t3f_current_view->flags & T3F_FILL_SCREEN)
		{
			...
			al_build_transform(&t3f_current_transform, t3f_display_offset_x + ox * t3f_display_scale_x, t3f_display_offset_y + oy * t3f_display_scale_y, dsx * t3f_display_scale_x, dsy * t3f_display_scale_y, 0.0);
		}
		else
		{
			/* need to adjust y */
	...
	else
	{
		...
		al_build_transform(&t3f_current_transform, t3f_current_view->offset_x, t3f_current_view->offset_y, dsx, dsy, 0.0);
	}
...
//	al_build_transform(&t3f_current_transform, ox + (t3f_current_view->offset_x * dsx), oy + (t3f_current_view->offset_y * dsy), dsx * sx, dsy * sy, 0.0);
